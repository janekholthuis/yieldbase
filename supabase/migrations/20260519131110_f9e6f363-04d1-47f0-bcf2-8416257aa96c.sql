...
begin
  ...
  -- Mirror persönliche Daten in profiles (Single Source bleibt kunden)
...
-- Helper RPC für Step-Saves: spiegelt step1 Felder in profiles
...
DROP TRIGGER IF EXISTS trg_sync_kunde_to_profile ON public.kunden;
CREATE TRIGGER trg_sync_kunde_to_profile
AFTER UPDATE ON public.kunden
FOR EACH ROW EXECUTE FUNCTION public.sync_kunde_to_profile();